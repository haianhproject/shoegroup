...
/* [TOI UU] Khong hard-code mat khau DB nua - doc tu .env */
const dbConfig = {
  user: config.db.user,
...
/* [FIX] Them day du EmailVerified, PointBalance, CreatedAt vao dang ky */
app.post("/api/register", async (req, res) => {
  try {
    ...
    const r = await pool
      ...
      .query(
        "INSERT INTO Users (RoleID, FullName, Email, PasswordHash, IsActive, EmailVerified, PointBalance, CreatedAt) OUTPUT INSERTED.UserID as id_user, INSERTED.Email as email, INSERTED.FullName as full_name, INSERTED.Phone as phone, INSERTED.Address as address, INSERTED.RoleID as role_id VALUES (2, @f, @e, @p, 1, 1, 0, GETDATE())",
      );
...
/* [FIX] Them day du EmailVerified, PointBalance, CreatedAt vao tao tai khoan */
app.post("/api/accounts", async (req, res) => {
  try {
    ...
    await pool
      ...
      .query(
        "INSERT INTO Users (Email, PasswordHash, FullName, RoleID, IsActive, EmailVerified, PointBalance, CreatedAt) VALUES (@e, @p, @n, @r, 1, 1, 0, GETDATE())",
      );
    res.json({ success: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
...
/* [FIX] Them day du EmailVerified, PointBalance vao tao khach vang lai */
app.post("/api/customers", async (req, res) => {
  try {
    ...
    let r = await pool
      ...
      .query(
        "INSERT INTO Users (RoleID, FullName, Phone, Email, PasswordHash, Address, Source, IsActive, EmailVerified, PointBalance, CreatedAt) OUTPUT INSERTED.UserID VALUES (2, @f, @ph, @e, @p, @a, @s, 1, 1, 0, GETDATE())",
      );
...
app.post("/api/auth/forgot-password", async (req, res) => {
  try {
    ...
    if (r.recordset.length === 0) return res.json({ success: true });
...
/* ===== [TOI UU] 404 + xu ly loi tap trung ===== */
