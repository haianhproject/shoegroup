  ...
  <div class="fade-in">
    <div class="bg-white rounded-4 shadow-sm p-4">
      ...
      <div class="d-flex flex-wrap justify-content-between align-items-start gap-2 mb-4">
        ...
        <button @click="openDiscountForm()" class="btn btn-dark rounded-3 fw-bold shadow-sm px-3"><i class="bi bi-plus-lg me-1"></i> Thêm mã giảm giá</button>
      </div>
      ...
      <div class="d-flex flex-wrap gap-2 mb-3">
        <div class="position-relative flex-grow-1" style="min-width:240px;max-width:420px;">
          ...
          <input v-model="discountSearch" type="text" class="form-control rounded-3 ps-4" placeholder="Tìm theo mã hoặc tên chương trình">
        </div>
        <select v-model="discountStatusFilter" class="form-select rounded-3" style="max-width:200px;">
        ...
        <table class="table align-middle mb-0">
          ...
          <tbody>
            ...
            <tr v-for="d in filteredDiscountsList" :key="d.id">
              ...
              <td><span class="badge rounded-pill bg-light text-secondary border" v-text="d.discount_type"></span></td>
    ...
    <div v-if="discountModal.open" class="custom-modal-overlay" @click.self="closeDiscountForm()">
      <div class="custom-modal-box fade-in-scale" style="max-width:640px;">
        ...
        <div class="p-4" style="max-height:65vh;overflow:auto;">
          <div class="row g-3">
            <div class="col-md-6"><label class="form-label small fw-medium">Mã giảm giá</label><input v-model="discountModal.data.code" type="text" class="form-control rounded-3" placeholder="VD: NEW200"></div>
            <div class="col-md-6"><label class="form-label small fw-medium">Tên chương trình</label><input v-model="discountModal.data.name" type="text" class="form-control rounded-3" placeholder="VD: Giảm giá sập sàn"></div>
            <div class="col-md-6"><label class="form-label small fw-medium">Loại giảm giá</label><select v-model="discountModal.data.discount_type" class="form-select rounded-3"><option v-for="t in discountTypes" :key="t" :value="t" v-text="t"></option></select></div>
            <div class="col-md-6"><label class="form-label small fw-medium">Giá trị giảm</label><input v-model.number="discountModal.data.value" type="number" class="form-control rounded-3" placeholder="0"></div>
            <div class="col-md-6"><label class="form-label small fw-medium">Đơn tối thiểu</label><input v-model.number="discountModal.data.min_order" type="number" class="form-control rounded-3" placeholder="0"></div>
            <div class="col-md-6"><label class="form-label small fw-medium">Giảm tối đa</label><input v-model.number="discountModal.data.max_discount" type="number" class="form-control rounded-3" placeholder="0"></div>
            <div class="col-md-6"><label class="form-label small fw-medium">Số lượng</label><input v-model.number="discountModal.data.quantity" type="number" class="form-control rounded-3" placeholder="0"></div>
            <div class="col-md-6"><label class="form-label small fw-medium">Ngày bắt đầu</label><input v-model="discountModal.data.start_date" type="date" class="form-control rounded-3"></div>
            <div class="col-md-6"><label class="form-label small fw-medium">Ngày kết thúc</label><input v-model="discountModal.data.expiry" type="date" class="form-control rounded-3"></div>
            <div class="col-12"><label class="form-label small fw-medium">Mô tả (tùy chọn)</label><textarea v-model="discountModal.data.description" rows="2" class="form-control rounded-3"></textarea></div>
        ...
        <div class="p-4 border-top d-flex justify-content-end gap-2"><button @click="closeDiscountForm()" class="btn btn-light border rounded-3">Hủy</button><button @click="saveDiscount" class="btn btn-dark rounded-3 fw-bold">Lưu</button></div>
      </div>
    </div>
  </div>
</template>
