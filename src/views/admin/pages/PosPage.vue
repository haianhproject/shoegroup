      ...
      <div class="col-lg-7">
        ...
        <div class="bg-white rounded-1 shadow-sm p-4 mb-4">
          ...
          <div class="d-flex gap-2 mb-3">
            <button @click="activePosOrder.customer_type = 'Có tài khoản'" class="btn btn-sm rounded-1 px-3 border" :class="activePosOrder.customer_type === 'Có tài khoản' ? 'btn-dark text-white border-dark' : 'btn-white text-secondary'">Có tài khoản</button>
            <button @click="activePosOrder.customer_type = 'Khách lẻ'" class="btn btn-sm rounded-1 px-3 border" :class="activePosOrder.customer_type === 'Khách lẻ' ? 'btn-dark text-white border-dark' : 'btn-white text-secondary'">Khách lẻ</button>
          </div>
          ...
          <div v-if="activePosOrder.customer_type === 'Khách lẻ'" class="row g-3">
            <div class="col-md-6"><label class="form-label small fw-medium text-uppercase text-secondary">Tên khách</label><input v-model="activePosOrder.customer_name" type="text" class="form-control rounded-2" placeholder="Họ tên khách hàng"></div>
            <div class="col-md-6"><label class="form-label small fw-medium text-uppercase text-secondary">Số điện thoại</label><input v-model="activePosOrder.customer_phone" type="tel" class="form-control rounded-2" placeholder="VD: 0901234567" maxlength="11"></div>
            <div class="col-12"><label class="form-label small fw-medium text-uppercase text-secondary">Ghi chú</label><textarea v-model="activePosOrder.customer_note" rows="2" class="form-control rounded-2" placeholder="Ghi chú đơn hàng..."></textarea></div>
          ...
          <div v-else>
            ...
            <div class="position-relative mb-2">
              ...
              <input v-model="posCustomerSearch" type="text" class="form-control rounded-2 ps-5" placeholder="Nhập tên hoặc SĐT để tìm...">
            </div>
        ...
        <div class="bg-white rounded-1 shadow-sm p-4">
          ...
          <div class="position-relative mb-3">
            <i class="bi bi-search position-absolute text-secondary" style="left:12px;top:50%;transform:translateY(-50%);"></i>
            <input v-model="posSearch" type="text" class="form-control rounded-2 ps-5" placeholder="Tìm theo tên, mã, màu, size...">
          </div>
            ...
            <div v-for="v in posVariants" :key="v.id" class="col-6 col-md-4">
              <div class="border rounded-2 p-2 h-100 d-flex flex-column">
                ...
                <div class="d-flex gap-1 mt-auto">
                  <input type="number" min="1" v-model.number="qtyInputs[v.id]" class="form-control form-control-sm text-center rounded-2" style="width:52px;" placeholder="1">
                  <button @click="addWithQty(v)" :disabled="v.stock <= 0" class="btn btn-sm btn-dark rounded-2 flex-grow-1"><i class="bi bi-plus-lg"></i> Thêm</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="bg-white rounded-1 shadow-sm p-4">
          <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <div class="d-flex align-items-center gap-2">
              <span class="badge rounded-1 bg-light text-secondary border" v-text="'#' + activePosOrder.code"></span>
          ...
          <div class="row g-2 mb-3">
            <div class="col-4"><div class="bg-light-gray rounded-2 p-2"><div class="text-secondary text-uppercase" style="font-size:0.62rem;">Khách hàng</div><div class="small fw-medium text-truncate" v-text="activePosOrder.customer_name || 'Khách lẻ'"></div></div></div>
            <div class="col-4"><div class="bg-light-gray rounded-2 p-2"><div class="text-secondary text-uppercase" style="font-size:0.62rem;">SĐT</div><div class="small fw-medium text-truncate" v-text="activePosOrder.customer_phone || '—'"></div></div></div>
            <div class="col-4"><div class="bg-light-gray rounded-2 p-2"><div class="text-secondary text-uppercase" style="font-size:0.62rem;">Loại đơn</div><div class="small fw-medium">TẠI QUẦY</div></div></div>
          </div>

          <div class="text-secondary text-uppercase mb-2" style="font-size:0.68rem;">Sản phẩm trong đơn</div>
          <div v-if="activePosOrder.cart.length === 0" class="text-center text-secondary small py-3 border rounded-2 mb-3">Chưa có sản phẩm trong đơn</div>
          <div v-else class="mb-3" style="max-height:200px;overflow:auto;">
            <div v-for="(c, i) in activePosOrder.cart" :key="i" class="d-flex align-items-center gap-2 py-2 border-bottom">
              ...
              <input type="number" min="1" @change="validateCartItemQty(c)" v-model.number="c.quantity" class="form-control form-control-sm text-center rounded-2" style="width:56px;">
          ...
          <div class="d-flex gap-2 mb-3">
            <select v-model="activePosOrder.coupon_code" class="form-select form-select-sm rounded-2">
          ...
          <div class="row g-2 mb-3">
            <div class="col-6"><button @click="activePosOrder.payment_method = 'Tiền mặt'" class="btn w-100 rounded-2 border py-2" :class="activePosOrder.payment_method === 'Tiền mặt' ? 'btn-dark text-white border-dark' : 'btn-white text-secondary'"><i class="bi bi-cash-coin me-1"></i>Tiền mặt</button></div>
            <div class="col-6"><button @click="activePosOrder.payment_method = 'Chuyển khoản'" class="btn w-100 rounded-2 border py-2" :class="activePosOrder.payment_method === 'Chuyển khoản' ? 'btn-dark text-white border-dark' : 'btn-white text-secondary'"><i class="bi bi-bank me-1"></i>Chuyển khoản</button></div>
          </div>

          <button @click="checkoutPos()" :disabled="activePosOrder.cart.length === 0" class="btn btn-dark w-100 rounded-2 fw-bold py-2"><i class="bi bi-check2-circle me-2"></i>Tạo đơn / Thanh toán</button>
        </div>
      </div>
    </div>
        ...
        <div class="custom-modal-box fade-in-scale" style="max-width:380px;">
          <div class="p-4 text-center">
            ...
            <div class="d-grid gap-2">
              <button @click="confirmPosPaid()" class="btn btn-dark rounded-2 fw-bold py-2"><i class="bi bi-check2-circle me-2"></i>Đã thanh toán</button>
