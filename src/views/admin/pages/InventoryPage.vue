...
<template>
  <div class="fade-in">
    ...
    <div class="row g-3 mb-4">
      <div class="col-6 col-lg-3"><div class="bg-white rounded-1 shadow-sm p-3"><p class="text-secondary small mb-1">Tổng biến thể</p><h5 class="fw-bold mb-0" v-text="db.inventory.length"></h5></div></div>
      <div class="col-6 col-lg-3"><div class="bg-white rounded-1 shadow-sm p-3"><p class="text-secondary small mb-1">Tổng tồn kho</p><h5 class="fw-bold mb-0" v-text="db.inventory.reduce((s,v)=>s+(Number(v.stock)||0),0)"></h5></div></div>
      <div class="col-6 col-lg-3"><div class="bg-white rounded-1 shadow-sm p-3"><p class="text-secondary small mb-1">Sắp hết hàng</p><h5 class="fw-bold mb-0 text-dark" v-text="db.inventory.filter(v=>Number(v.stock)>0 && Number(v.stock)<=LOW_STOCK_THRESHOLD).length"></h5></div></div>
      <div class="col-6 col-lg-3"><div class="bg-white rounded-1 shadow-sm p-3"><p class="text-secondary small mb-1">Hết hàng</p><h5 class="fw-bold mb-0 text-danger" v-text="db.inventory.filter(v=>Number(v.stock)<=0).length"></h5></div></div>
    </div>

    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-4">
      <div class="input-group bg-white rounded-2 shadow-sm" style="max-width:320px;"><span class="input-group-text bg-white border-0"><i class="bi bi-search text-secondary"></i></span><input v-model="inventorySearch" type="text" class="form-control border-0" placeholder="Tìm theo SKU / tên..."></div>
            ...
            <tr v-for="v in filteredInventory" :key="v.id">
              ...
              <td class="text-end pe-4">
                <div class="d-inline-flex gap-1 align-items-center">
                  <button @click="stepStock(v,-1)" class="btn btn-sm btn-light border rounded-2" :disabled="Number(v.stock)<=0"><i class="bi bi-dash"></i></button>
                  ...
                  <button @click="stepStock(v,1)" class="btn btn-sm btn-light border rounded-2"><i class="bi bi-plus"></i></button>
                  <button @click="updateStock(v)" class="btn btn-sm btn-dark rounded-2" title="Lưu tồn kho"><i class="bi bi-check-lg"></i></button>
                </div>
              </td>
            </tr>
