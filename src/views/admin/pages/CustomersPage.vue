...
<template>
  <div class="fade-in">
    <div class="input-group bg-white rounded-2 shadow-sm mb-4" style="max-width:320px;"><span class="input-group-text bg-white border-0"><i class="bi bi-search text-secondary"></i></span><input v-model="customerSearch" type="text" class="form-control border-0" placeholder="Tìm khách hàng..."></div>
    <div class="row g-3">
      <div v-for="cus in filteredCustomers" :key="cus.id" class="col-12 col-md-6 col-xl-4">
        <div class="bg-white p-4 h-100" style="border-radius:4px;">
          <div class="d-flex align-items-center gap-3 mb-3">
            <div class="bg-dark text-white d-flex align-items-center justify-content-center fw-bold fs-5" style="width:44px;height:44px;border-radius:4px;flex-shrink:0;" v-text="(cus.name || '?').charAt(0).toUpperCase()"></div>
          ...
          <div class="d-flex justify-content-between align-items-center bg-light-gray p-2 px-3 mb-3" style="border-radius:3px;"><span class="text-secondary small">Tổng chi tiêu</span><span class="fw-bolder text-dark" v-text="formatPrice(cus.spent)"></span></div>
          <button @click="viewCustomerDetails(cus)" class="btn btn-sm btn-outline-dark w-100" style="border-radius:4px;"><i class="bi bi-eye me-1"></i> Xem chi tiết</button>
        </div>
      </div>
    </div>
        ...
        <div class="p-4" style="max-height:72vh;overflow:auto;">
          <div v-if="customerModal.customer" class="d-flex align-items-center gap-3 mb-3">
            ...
            <div class="flex-grow-1">
              ...
              <span v-if="customerModal.customer.source" class="badge rounded-1 bg-light text-secondary border mt-1" v-text="'Nguồn: ' + customerModal.customer.source"></span>
            </div>
            <span class="badge align-self-start" style="border-radius:2px;font-size:0.72rem;" :class="getRank(customerModal.customer.spent).class" v-text="getRank(customerModal.customer.spent).label"></span>
          </div>

          <div class="row g-2 mb-4">
            <div class="col-6"><div class="bg-light-gray rounded-2 p-3 text-center"><div class="text-secondary text-uppercase" style="font-size:0.68rem;">Tổng chi tiêu</div><div class="fw-bolder text-dark fs-5" v-text="formatPrice(customerModal.customer ? customerModal.customer.spent : 0)"></div></div></div>
            <div class="col-6"><div class="bg-light-gray rounded-2 p-3 text-center"><div class="text-secondary text-uppercase" style="font-size:0.68rem;">Số đơn hàng</div><div class="fw-bolder text-dark fs-5" v-text="customerModal.orders.length"></div></div></div>
          </div>

          <h6 class="fw-bold text-dark mb-2"><i class="bi bi-bag-check me-2"></i>Lịch sử đơn hàng</h6>
          <div v-if="customerModal.orders.length === 0" class="text-center text-secondary py-4 small border rounded-2">Khách chưa có đơn hàng nào.</div>
