...
<template>
  <div class="fade-in">
    ...
    <div class="bg-white rounded-4 shadow-sm p-4 mb-4">
      ...
      <div class="d-flex flex-wrap align-items-center gap-2">
        ...
        <div class="position-relative flex-grow-1" style="min-width:240px;max-width:440px;">
          ...
          <input v-model="returnSearchCode" @keyup.enter="searchReturnOrder()" type="text" class="form-control rounded-3 ps-4" placeholder="VD: SGVN..., HD13 hoặc mã đơn">
        </div>
        <button @click="searchReturnOrder()" class="btn btn-dark rounded-3 fw-bold px-3"><i class="bi bi-search me-1"></i> Tìm kiếm</button>
        <button @click="notify('Đưa mã vào ô rồi bấm Tìm kiếm', 'info')" class="btn btn-outline-dark rounded-3 fw-bold px-3"><i class="bi bi-upc-scan me-1"></i> Quét mã</button>
      </div>
    </div>

    <!-- Chưa tìm thấy: trạng thái rỗng -->
    <div v-if="!returnFoundOrder" class="bg-white rounded-4 shadow-sm p-5 text-center mb-4">
      ...
      <div class="col-lg-8">
        <div class="bg-white rounded-4 shadow-sm p-4">
          <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <span class="badge rounded-pill bg-light text-secondary border" v-text="'Đơn #' + returnFoundOrder.id"></span>
          </div>
            ...
            <table class="table align-middle mb-0">
              ...
              <tbody>
                ...
                <tr v-for="it in returnItems" :key="it.idx">
                  ...
                  <td class="text-center"><input type="number" min="0" :max="it.max" v-model.number="it.return_qty" class="form-control form-control-sm text-center rounded-3"></td>
          ...
          <div class="mt-3">
            ...
            <textarea v-model="returnNote" rows="3" class="form-control rounded-3" placeholder="Lý do trả hàng, tình trạng sản phẩm..."></textarea>
          </div>
        </div>
      </div>
      <div class="col-lg-4">
        <div class="bg-white rounded-4 shadow-sm p-4">
          ...
          <button @click="submitReturn()" :disabled="returnSelectedCount === 0" class="btn btn-dark w-100 rounded-3 fw-bold py-2"><i class="bi bi-arrow-return-left me-2"></i>Trả hàng</button>
          <button @click="resetReturnForm()" class="btn btn-light border w-100 rounded-3 mt-2">Hủy</button>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white rounded-4 shadow-sm p-4">
      <h6 class="fw-bold mb-3 text-dark"><i class="bi bi-clock-history me-2"></i>Yêu cầu trả hàng</h6>
