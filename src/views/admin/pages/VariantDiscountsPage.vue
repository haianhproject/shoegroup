  ...
  <div class="fade-in">
    <div class="bg-white rounded-4 shadow-sm p-4">
      ...
      <div class="d-flex flex-wrap justify-content-between align-items-start gap-2 mb-4">
        ...
        <button @click="openVariantDiscountForm()" class="btn btn-dark rounded-3 fw-bold shadow-sm px-3"><i class="bi bi-plus-lg me-1"></i> Thêm Giảm Giá</button>
      </div>
      ...
      <div class="d-flex flex-wrap gap-2 mb-3">
        <div class="position-relative flex-grow-1" style="min-width:220px;max-width:360px;">
          ...
          <input v-model="variantDiscountSearch" type="text" class="form-control rounded-3 ps-4" placeholder="Tìm theo sản phẩm / màu">
        </div>
        <select v-model="variantReasonFilter" class="form-select rounded-3" style="max-width:200px;">
          <option value="Tất cả">Lọc theo lý do</option>
          <option v-for="r in variantReasons" :key="r" :value="r" v-text="r"></option>
        </select>
        <select v-model="variantStatusFilter" class="form-select rounded-3" style="max-width:200px;">
        ...
        <table class="table align-middle mb-0">
          ...
          <tbody>
            ...
            <tr v-for="vd in filteredVariantDiscounts" :key="vd.id">
              <td><img :src="getVariantInfo(vd).image || 'https://via.placeholder.com/44'" class="rounded-3 border" style="width:44px;height:44px;object-fit:cover;" @error="$event.target.src='https://via.placeholder.com/44'"></td>
              <td>
                ...
                <span v-if="vd.reason" class="badge rounded-pill bg-light text-secondary border mt-1" style="font-size:0.68rem;" v-text="vd.reason"></span>
              </td>
              ...
              <td><span class="badge rounded-pill" :class="vd.discount_type === 'Cố định' ? 'bg-success-subtle text-success-emphasis' : 'bg-info-subtle text-info-emphasis'" v-text="vd.discount_type"></span></td>
              ...
              <td><span class="badge rounded-pill" :class="getVariantDiscountStatus(vd).cls" v-text="getVariantDiscountStatus(vd).label"></span></td>
              <td class="text-end">
                <button @click="openVariantDiscountForm(vd)" class="btn btn-sm btn-light border rounded-3 me-1"><i class="bi bi-pencil"></i></button>
                <button @click="deleteItem('variantDiscounts', vd.id, getVariantInfo(vd).product_name)" class="btn btn-sm btn-light border rounded-3 text-danger"><i class="bi bi-trash"></i></button>
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div class="custom-modal-box fade-in-scale" style="max-width:640px;">
        ...
        <div class="p-4" style="max-height:65vh;overflow:auto;">
          ...
          <div class="mb-3">
            ...
            <select v-model="variantDiscountModal.data.variant_id" class="form-select rounded-3">
          ...
          <!-- Cảnh báo trùng -->
          <div v-if="variantAlreadyDiscounted(variantDiscountModal.data.variant_id) && !variantDiscountModal.data.id" class="alert alert-warning py-2 px-3 small rounded-3 d-flex align-items-start gap-2">
          ...
          <div class="row g-3">
            <div class="col-md-6"><label class="form-label small fw-medium">Loại giảm giá</label><select v-model="variantDiscountModal.data.discount_type" class="form-select rounded-3"><option v-for="t in variantDiscountTypes" :key="t" :value="t" v-text="t"></option></select></div>
            <div class="col-md-6"><label class="form-label small fw-medium">Giá trị giảm</label><input v-model.number="variantDiscountModal.data.value" type="number" class="form-control rounded-3" placeholder="Nhập số tiền (VNĐ) hoặc %"><small class="text-secondary" style="font-size:0.72rem;">Cố định: nhập số tiền · Phần trăm: nhập %</small></div>
            <div class="col-md-6"><label class="form-label small fw-medium">Giảm tối đa (nếu %)</label><input v-model.number="variantDiscountModal.data.max_discount" type="number" class="form-control rounded-3" placeholder="0"></div>
            <div class="col-md-6"><label class="form-label small fw-medium">Số lượng</label><input v-model.number="variantDiscountModal.data.quantity" type="number" class="form-control rounded-3" placeholder="Phải lớn hơn 0"></div>
            <div class="col-md-6"><label class="form-label small fw-medium">Ngày bắt đầu</label><input v-model="variantDiscountModal.data.start_date" type="date" class="form-control rounded-3"></div>
            <div class="col-md-6"><label class="form-label small fw-medium">Ngày kết thúc</label><input v-model="variantDiscountModal.data.end_date" type="date" class="form-control rounded-3"></div>
            <div class="col-md-6"><label class="form-label small fw-medium">Lý do giảm giá</label><select v-model="variantDiscountModal.data.reason" class="form-select rounded-3"><option v-for="r in variantReasons" :key="r" :value="r" v-text="r"></option></select></div>
            ...
            <div class="col-12"><label class="form-label small fw-medium">Mô tả (tùy chọn)</label><textarea v-model="variantDiscountModal.data.description" rows="2" class="form-control rounded-3"></textarea></div>
          </div>
        </div>
        <div class="p-4 border-top d-flex justify-content-end gap-2"><button @click="closeVariantDiscountForm()" class="btn btn-light border rounded-3">Hủy</button><button @click="saveVariantDiscount" class="btn btn-dark rounded-3 fw-bold">Lưu</button></div>
      </div>
    </div>
  </div>
</template>
