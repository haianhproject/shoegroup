...
const openEdit = async (a) => {
  ...
  if (modal.provinceId) {
    loadingCommunes.value = true
    try {
      const res = await fetch(`https://provinces.open-api.vn/api/v2/w/?province=${modal.provinceId}`)
      if (res.ok) communes.value = await res.json()
    } finally {
      loadingCommunes.value = false
    }
    modal.communeId = a.communeId || ''
  }
}
...
const saveAddr = () => {
  ...
  const fullAddr = `${modal.line.trim()}, ${modal.communeName}, ${modal.provinceName}`
  ...
  if (modal.editId) {
    ...
    if (a) {
      Object.assign(a, { 
        ...
        phone: cleanPhone, 
        provinceId: modal.provinceId, provinceName: modal.provinceName,
        communeId: modal.communeId, communeName: modal.communeName,
      ...
      <div class="row g-4">
        ...
        <div class="col-lg-9 acc-content">
          ...
          <div v-else-if="tab === 'address'" class="sg-card acc-block">
            ...
            <div v-if="addresses.length === 0" class="text-center py-5 text-muted empty-state">
              ...
              <div v-for="a in addresses" :key="a.id" class="addr-card" :class="{ def: a.isDefault }">
                <div class="flex-grow-1">
                  ...
                  <div class="text-secondary mt-1 addr-detail-text">
                    {{ a.fullAddress || `${a.line}, ${a.communeName || a.ward || ''}, ${a.provinceName || a.province || ''}` }}
                  </div>
                </div>
      ...
      <div v-if="modal.open" class="modal-overlay" @click.self="closeModal">
        <div class="sg-card modal-box">
          ...
          <div class="row g-3">
            ...
            <div class="col-12">
              <label class="check-row">
                <input type="checkbox" v-model="modal.isDefault"> 
          ...
          <div class="d-flex gap-2 mt-4 justify-content-end">
            ...
            <button class="btn-sg" @click="saveAddr"><i class="bi bi-check-lg me-1"></i>Lưu địa chỉ</button>
          </div>
        </div>
      </div>
    </transition>
