...
const selectProvince = async (p) => {
  ...
  try {
    const res = await fetch(`https://provinces.open-api.vn/api/v2/w/?province=${p.code}`)
    if (res.ok) communes.value = await res.json()
  } finally {
...
const saveNewAddress = () => {
  ...
  if (addrModal.isDefault) savedAddresses.value.forEach(a => a.isDefault = false)

  const fullAddr = `${addrModal.line.trim()}, ${addrModal.communeName}, ${addrModal.provinceName}`
  const newAddr = {
    id: Date.now(),
    recipient: addrModal.recipient.trim(),
    phone: cleanPhone,
    provinceId: addrModal.provinceId,
    provinceName: addrModal.provinceName,
    province: addrModal.provinceName,
    communeId: addrModal.communeId,
    communeName: addrModal.communeName,
...
const addressVerified = computed(() => {
  ...
  return /\d+/.test(a) && a.length >= 10 && form.province.trim().length >= 2
})

const mapUrl = computed(() => {
  const query = encodeURIComponent(`${form.address}, ${form.province}, Việt Nam`)
        ...
        <div class="col-lg-7">
          ...
          <div class="co-block">
            ...
            <div class="mt-4">
              ...
              <select v-model="selectedAddressId" class="sg-input w-100 bg-light fw-semibold text-dark">
                <option :value="null" disabled>-- Chọn địa chỉ đã lưu trong sổ --</option>
                <option v-for="a in savedAddresses" :key="a.id" :value="a.id">
                  {{ a.recipient || a.fullName }} - {{ a.phone }} ({{ a.fullAddress || `${a.line}, ${a.communeName || a.ward || ''}, ${a.provinceName || a.province || ''}` }}) {{ a.isDefault ? ' [⭐ Mặc định]' : '' }}
                </option>
              </select>
            </div>
            ...
            <div v-if="selectedAddress" class="mt-3 p-3 rounded" style="background: #f8fafc; border: 1px solid #e2e8f0;">
                ...
                <div class="text-secondary small">
                  {{ selectedAddress.fullAddress || `${selectedAddress.line}, ${selectedAddress.communeName || selectedAddress.ward || ''}, ${selectedAddress.provinceName || selectedAddress.province || ''}` }}
                </div>
            </div>
      ...
      <div v-if="addrModal.open" class="modal-overlay" @click.self="addrModal.open = false">
        <div class="sg-card modal-box">
          ...
          <div class="d-flex gap-2 mt-4 justify-content-end">
            ...
            <button class="btn-sg" @click="saveNewAddress"><i class="bi bi-save me-1"></i>Lưu & Chọn</button>
          </div>
        </div>
      </div>
    </transition>
